...
*/
#ifdef USE_OMP_ACCELERATOR
#pragma omp target teams distribute parallel for reduction(+ : scout_count) nowait
#else
#pragma omp  parallel for reduction(+ : scout_count) nowait
#endif
...
/**
  for (NodeID n = 0; n < g.num_nodes(); n++)
    if (parent->operator[](n) != -1)
     cout << "---parent---" <<parent->operator[](n) << endl;
  */
  return parent;
}
...
bool BFSVerifier(const Graph &g, NodeID source,
  ...
  for (NodeID u : g.vertices()) {
    if ((depth[u] != -1) && (parent->operator[](u) != -1)) {
      if (u == source) {
        if (!((parent->operator[](u) == u) && (depth[u] == 0))) {
      ...
      if (!parent_found) {
        cout << "Couldn't find edge from " << parent->operator[](u) << " to " << u << endl;
        return false;
      }
    } else if (depth[u] != parent->operator[](u)) {
...
int main(int argc, char* argv[]) {
  CLApp *cli = new CLApp(argc, argv, "breadth-first search");
  if (!cli->ParseArgs())
    return -1;
  Builder b(*cli);
  Graph g = b.MakeGraph();
  SourcePicker<Graph> *sp = new SourcePicker<Graph>(g, cli->start_vertex());
  /**
